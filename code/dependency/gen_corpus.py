...
with open(input_file, encoding='utf-8') as in_file, \
        open(input_file + '.lemma', 'w', encoding='utf-8') as lemma_file, \
        open(input_file + '.pos', 'w', encoding='utf-8') as pos_file, \
        open(input_file + '.norm_termab', 'w', encoding='utf-8') as norm_termab_file:
    count = 0
    for line in in_file:
        count += 1
        if count % 1000 == 0:
            print(input_file + str(count))
        ls = line.strip().split('\t')
        document = ls[1].strip()
        pairs = json.loads(ls[3]) + json.loads(ls[4])
        if 5 <= len(document.split()) <= 25:
            ...
            pos_file.write(' '.join([pos[:2] for pos in pos_list]) + '\n')

            for pair in pairs:
                token1 = pair.split()[0]
                token2 = pair.split()[1]
                try:
                    token1_index = lemma_list.index(token1)
                    token2_index = lemma_list.index(token2)
                except Exception:
                    continue
